...
func (p *Processor) HandlePDUSessionSMContextCreate(
	...
	// Marshal and print the smPolicyDecision object for debugging
	if jsonData, err := json.MarshalIndent(smPolicyDecision, "", "  "); err != nil {
		logger.CtxLog.Errorf("Failed to marshal smPolicyDecision: %v", err)
	} else {
		logger.CtxLog.Infof("5g team This is the marshalled smPolicyDecision: %s", string(jsonData))
	}

	// Marshal and print the smPolicyID object for debugging
	if jsonData, err := json.MarshalIndent(smPolicyID, "", "  "); err != nil {
		logger.CtxLog.Errorf("Failed to marshal smPolicyID: %v", err)
	} else {
		logger.CtxLog.Infof("5g team This is the marshalled smPolicyID: %s", string(jsonData))
	}
	...
	// To print the smcontext PDR and QER for debugging
	// if smContext == nil {
	// 	fmt.Println("Error: smContext is nil")
	// } else if smContext.Tunnel == nil {
	// 	fmt.Println("Error: smContext.Tunnel is nil")
...
func (p *Processor) HandlePDUSessionSMContextUpdate(
	...
	if body.BinaryDataN1SmMessage != nil {
		...
		smContext.Log.Infof("GsmHeader.GetMessageType()-", m.GsmHeader.GetMessageType())
		switch m.GsmHeader.GetMessageType() {
		case nas.MsgTypePDUSessionReleaseRequest:
			smContext.Log.Infof("In MsgTypePDUSessionReleaseRequest case")
		...
		case nas.MsgTypePDUSessionModificationRequest:
			logger.PduSessLog.Infof("In MsgTypePDUSessionModificationRequest")
			if rsp, errHandleReq := p.
				...
				if buf, err = rsp.PlainNasEncode(); err != nil {
					...
					response.BinaryDataN1SmMessage = buf
					logger.PduSessLog.Infof("above sendGSMPDUSessionModificationCommand")
		...
		case nas.MsgTypePDUSessionModificationComplete:
			logger.PduSessLog.Infoln("In MsgTypePDUSessionModificationComplete")
			smContext.StopT3591()
		case nas.MsgTypePDUSessionModificationReject:
			logger.PduSessLog.Infoln("In MsgTypePDUSessionModificationReject")
			smContext.StopT3591()
		}
	}
...
func (p *Processor) sendGSMPDUSessionModificationCommand(smContext *smf_context.SMContext, nasPdu []byte) {
	smContext.Log.Tracef("Prepare to send GSMPDUSessionModificationCommand")
	n1n2Request := models.N1N2MessageTransferRequest{}
	n1n2Request.JsonData = &models.N1N2MessageTransferReqData{
		PduSessionId: smContext.PDUSessionID,
		N1MessageContainer: &models.N1MessageContainer{
	...
	if t3591.Enable {
		...
		smContext.T3591 = smf_context.NewTimer(t3591.ExpireTime, t3591.MaxRetryTimes, func(expireTimes int32) {
			smContext.SMLock.Lock()
			defer smContext.SMLock.Unlock()
			smContext.Log.Tracef("Sending N1N2 message with PDU Session ID %d", smContext.PDUSessionID)

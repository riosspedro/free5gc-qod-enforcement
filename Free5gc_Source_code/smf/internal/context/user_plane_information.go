...
func (upi *UserPlaneInformation) UpNodesFromConfiguration(upTopology *factory.UserPlaneInformation) {
	for name, node := range upTopology.UPNodes {
		...
		case UPNODE_UPF:
			...
			logger.CtxLog.Infof("5g team This is the upf ID [%s]\n", upfid)
			logger.CtxLog.Info("5g team This is the upf ID w/o f \n", upfid)
			upfip := upNode.NodeID.ResolveNodeIdToIp().String()
			logger.CtxLog.Infof("5g team This is the upf IP [%s]\n", upfip)
			logger.CtxLog.Info("5g team This is the upf IP w/o f \n", upfid)
...
func (upi *UserPlaneInformation) selectMatchUPF(selection *UPFSelectionParams) []*UPNode {
	...
	logger.CtxLog.Infof("5g team This is the up list[%s]\n", upList)
	for _, upNode := range upi.UPFs {
		for _, snssaiInfo := range upNode.UPF.SNssaiInfos {
			currentSnssai := snssaiInfo.SNssai
			targetSnssai := selection.SNssai
			logger.CtxLog.Infof("5g team This is the current Snssai[%s]\n", currentSnssai)
			if currentSnssai.Equal(targetSnssai) {
				for _, dnnInfo := range snssaiInfo.DnnList {
					logger.CtxLog.Infof("5g team This is the DNN INFO [%s]\n",dnnInfo.Dnn)
	...
	logger.CtxLog.Infof("5g team This is the up list while returning [%s]\n", upList)
	return upList
}

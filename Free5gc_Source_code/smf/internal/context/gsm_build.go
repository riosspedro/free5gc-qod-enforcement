...
func BuildGSMPDUSessionEstablishmentAccept(smContext *SMContext) ([]byte, error) {
	...
	logger.GsmLog.Infof("buildpduestablishment PTI[%d]", pDUSessionEstablishmentAccept.GetPTI())		
	...
	// 	fmt.Printf("PCC Rule: ID=%s, QFI=%d, Precedence=%d\n", pccRule.PccRuleId, pccRule.QFI, pccRule.Precedence)
	// 	for i, flow := range pccRule.FlowInfos {
	// 		fmt.Printf("FlowInfo[%d]: %+v\n", i, flow)
	// 	}
	...
	// 		}
	// 		logger.GsmLog.Infof("qosrule created : %+v", *qosRule)
	// 		qoSRules = append(qoSRules, *qosRule)
	// 		logger.GsmLog.Infof("QosruleList : %+v", qoSRules)
	// 		logger.GsmLog.Infof("Total QoS rules: %d", len(qoSRules))
	// 		for i, rule := range qoSRules {
	// 			logger.GsmLog.Infof("QoS Rule [%d]: %+v", i, rule)
	// 		}
...
func BuildGSMPDUSessionReleaseCommand(smContext *SMContext, cause uint8, isTriggeredByUE bool) ([]byte, error) {
	...
	logger.GsmLog.Infof("buildpdurelease PTI[%d]", pDUSessionReleaseCommand.GetPTI())
...
func BuildGSMPDUSessionModificationCommand(smContext *SMContext) ([]byte, error) {
	...
	logger.GsmLog.Infof("buildpdumodification PTI[%d]", pDUSessionModificationCommand.GetPTI())

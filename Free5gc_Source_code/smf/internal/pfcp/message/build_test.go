...
func TestBuildPfcpSessionModificationRequestRemoveQER(t *testing.T) {
	initSmfContext()
	...
	assert.Len(t, req.CreateQER, 0)
	assert.Len(t, req.RemoveQER, 1)
	assert.NotNil(t, req.RemoveQER[0])
	assert.NotNil(t, req.RemoveQER[0].QERID)
	assert.Equal(t, uint32(321), req.RemoveQER[0].QERID.QERID)
	assert.Equal(t, context.RULE_REMOVE, qer.State)
}

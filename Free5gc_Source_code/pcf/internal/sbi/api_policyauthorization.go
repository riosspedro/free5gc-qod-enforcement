...
func (s *Server) HTTPPostAppSessions(c *gin.Context) {
	...
	logger.PolicyAuthLog.Infof("🔎 POST /app-sessions triggered by: %s", c.ClientIP())
	...
	logger.CtxLog.Infof("Raw JSON string: %s\n", string(requestBody))
	...
	prettyJson, err := json.MarshalIndent(appSessionContext, "", "  ")
	if err != nil {
		logger.CtxLog.Errorln("Failed to marshal appSessionContext:", err)
	} else {
		logger.CtxLog.Infof("Deserialized appSessionContext:\n%s", string(prettyJson))
	}
...
func (s *Server) HTTPUpdateEventsSubsc(c *gin.Context) {
	...
	logger.CtxLog.Infof("Raw JSON string in patch: %s\n", string(requestBody))
...
// ----
	...
	prettyJson, err := json.MarshalIndent(appSessionContextUpdateData, "", "  ")
	if err != nil {
		logger.CtxLog.Errorln("Failed to marshal appSessionContextUpdateData:", err)
	} else {
	...
	if appSessionContextUpdateData.AfRoutReq != nil {
		logger.CtxLog.Infof("AfRoutingRequirementRm content: %+v", *appSessionContextUpdateData.AfRoutReq)
	} else {
		logger.CtxLog.Infof("AfRoutingRequirementRm is nil")
	}

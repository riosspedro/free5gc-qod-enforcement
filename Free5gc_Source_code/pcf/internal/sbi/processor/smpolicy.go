...
func (p *Processor) HandleCreateSmPolicyRequest(
	...
	if smPolicyData == nil || smPolicyData.SmPolicyData == nil {
		...
		// Marshal and print the smData object for debugging
		if jsonData, err := json.MarshalIndent(smData, "", "  "); err != nil {
			logger.CtxLog.Errorf("Failed to marshal smData: %v", err)
		} else {
			logger.CtxLog.Infof("5g team This is the marshalled smData: %s", string(jsonData))
		}
...
func (p *Processor) SendSMPolicyUpdateNotification(
	...
	logger.SmPolicyLog.Infof("Context: %+v", ctx)
	logger.SmPolicyLog.Infof("URI: %s", uri)
	if jsonData, err := json.MarshalIndent(req, "", "  "); err != nil {
		logger.SmPolicyLog.Errorf("Failed to marshal request: %v", err)
	} else {
		logger.SmPolicyLog.Infof("Request JSON: %s", string(jsonData))
	}

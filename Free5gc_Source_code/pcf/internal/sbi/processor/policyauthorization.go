...
func transferAfRoutReqRmToAfRoutReq(AfRoutReqRm *models.AfRoutingRequirementRm) *models.AfRoutingRequirement {
	...
	logger.CtxLog.Infof("5g team This is the af route request [%+v]\n",afRoutReq)
	return &afRoutReq
}
...
func transferMedCompRmToMedComp(medCompRm *models.MediaComponentRm) *models.MediaComponent {
	...
	// ✅ Add this log or nil check before using medCompRm.AfRoutReq
	if medCompRm.AfRoutReq == nil {
		logger.CtxLog.Warn("5g team AfRoutReq is nil in transferMedCompRmToMedComp")
	} else {
		logger.CtxLog.Infof("5g team AfRoutReq is not nil: %+v", medCompRm.AfRoutReq)
	}
	...
	logger.CtxLog.Infof("5g team This is the media components [%+v]\n", medComp)
	return &medComp
}

func printAllPccRules(pccRules map[string]*models.PccRule) {
    if len(pccRules) == 0 {
        logger.CtxLog.Infof("No PCC rules are present.")
        return
    }

    logger.CtxLog.Infof("Printing all PCC rules:")
    for id, rule := range pccRules {
        if jsonData, err := json.MarshalIndent(rule, "", "  "); err != nil {
            logger.CtxLog.Errorf("Failed to marshal PCC rule with ID [%s]: %v", id, err)
        } else {
            logger.CtxLog.Infof("PCC Rule ID [%s]: %s", id, string(jsonData))
        }
    }
}
...
func handleMediaSubComponent(smPolicy *pcf_context.UeSmPolicyData, medComp *models.MediaComponent,
	...
	if tempFlowInfos, err := getFlowInfos(medSubComp); err != nil {
	    ...
	    // Log all flowInfos for debugging
		if jsonData, err := json.MarshalIndent(flowInfos, "", "  "); err != nil {
			logger.CtxLog.Errorf("Failed to marshal flowInfos: %v", err)
		} else {
			logger.CtxLog.Infof("FlowInfos: %s", string(jsonData))
		}

		printAllPccRules(smPolicy.PolicyDecision.PccRules)
	pccRule := util.GetPccRuleByFlowInfos(smPolicy.PolicyDecision.PccRules, flowInfos)
	if pccRule == nil {
        logger.CtxLog.Infof("No matching PCC rule found for the given flowInfos.")
    } else {
        if jsonData, err := json.MarshalIndent(pccRule, "", "  "); err != nil {
            logger.CtxLog.Errorf("Failed to marshal pccRule: %v", err)
        } else {
            logger.CtxLog.Infof("PCC Rule: %s", string(jsonData))
        }
    }
	if pccRule == nil {
		...
		logger.CtxLog.Infof("5g team This is the qos data created when there is no pcc rule present[%+v]\n",qosData)
		...
		if var5qi <= 4 {
			...
			logger.CtxLog.Infof("5g team This is the qos data updated when var5qi is less than or equal to 4 [%+v]\n",qosData)
			logger.CtxLog.Infof("5g team This is the UL updated when var5qi is less than or equal to 4 [%+v]\n",ul)
			logger.CtxLog.Infof("5g team This is the DL updated when var5qi is less than or equal to 4 [%+v]\n",dl)
		...
		for _, qosID := range pccRule.RefQosData {
			...
			logger.CtxLog.Infof("5g team This is the qos data when there is pcc rule present[%+v]\n",qosData)
			if qosData.Var5qi == var5qi && qosData.Var5qi <= 4 {
				...
				logger.CtxLog.Infof("5g team This is the qos data when TWO conditions are true in line 112: qosData[%+v], ul[%+v], dl[%+v]\n", qosData, ul, dl)
	...
	logger.CtxLog.Infof("5g team This is pcc rules post updating qos line 125[%+v]\n",pccRule)
	return pccRule, nil
}
...
func (p *Processor) HandlePostAppSessionsContext(
	...
	logger.CtxLog.Infof("5g team This is the appsessionContext data [%+v]\n",appSessionContext)

	response, locationHeader, problemDetails := p.postAppSessCtxProcedure(&appSessionContext)

	logger.CtxLog.Infof("5g team This is the response data after procedure [%+v]\n",response)
	logger.CtxLog.Infof("5g team This is the location header data [%+v]\n",locationHeader)
...
func (p *Processor) postAppSessCtxProcedure(appSessCtx *models.AppSessionContext) (*models.AppSessionContext,
	...
	if ascReqData.BdtRefId != "" {
		...
		logger.CtxLog.Infof("5g team This is the app session id [%+v]\n",appSessID)
		data := pcf_context.AppSessionData{
			AppSessionId:      appSessID,
			AppSessionContext: appSessCtx,
		}

		logger.CtxLog.Infof("5g team This is the pcf context app session datadata line 184 [%+v]\n",data)
	...
	if ascReqData.MedComponents != nil {
		if jsonData, err := json.MarshalIndent(ascReqData.MedComponents, "", "  "); err != nil {
			logger.CtxLog.Errorf("5g team Failed to marshal MedComponents: %v", err)
		} else {
			logger.CtxLog.Infof("5g team This has MedComponents: %s", string(jsonData))
		}
		...
		for _, medComp := range ascReqData.MedComponents {
			...
			if medComp.MedSubComps != nil {
				logger.CtxLog.Infof("5g team This is having medsubcomponents ")
				...
				for _, medSubComp := range medComp.MedSubComps {
					if tempPccRule, problemDetail := handleMediaSubComponent(smPolicy,
						...
						return nil, "", problemDetail
						logger.CtxLog.Infof("5g team This was sent to handlemediasubcomponent line 263 and got temppccrule: [%+v]\n",tempPccRule)
			...
			logger.CtxLog.Infof("5g team This is pcc rule which is fetched using Af App ID line 288: [%+v]\n",pccRule)
			if pccRule == nil {
				logger.CtxLog.Infof("5g team This is pcc rule is not nill line 290: [%+v]\n",pccRule)

				pccRule = util.CreatePccRule(smPolicy.PccRuleIdGenerator, precedence, nil, appID)
				logger.CtxLog.Infof("5g team This is pcc rule created line 292: [%+v]\n",pccRule)
				// Set QoS Data
				// TODO: use real arp
				qosData := util.CreateQosData(smPolicy.PccRuleIdGenerator, var5qi, 8)
				logger.CtxLog.Infof("5g team This created qosdata 298 : [%+v]\n",qosData)

				logger.CtxLog.Infof("5g team This is the created qos data line 270: [%+v]\n",qosData)
				if var5qi <= 4 {
					// update Qos Data according to request BitRate
					var ul, dl bool
					qosData, ul, dl = updateQosInMedComp(qosData, &medComp)
					logger.CtxLog.Infof("5g team This is qos data updated line 305(secon if pccrule==nil loop): [%+v]\n",qosData)
					if problemDetails := modifyRemainBitRate(smPolicy, &qosData, ul, dl); problemDetails != nil {
						return nil, "", problemDetails
					}
				}
				...
				for _, qosID := range pccRule.RefQosData {
					...
					if qosData.Var5qi == var5qi && qosData.Var5qi <= 4 {
						...
						logger.CtxLog.Infof("5g team This is qos data updated in 2 if loop else statement line 323: [%+v]\n",qosData)
						
						if problemDetails := modifyRemainBitRate(smPolicy, &qosData, ul, dl); problemDetails != nil {
							return nil, "", problemDetails
						}
						smPolicy.PolicyDecision.QosDecs[qosData.QosId] = &qosData
						logger.CtxLog.Infof("5g team This is qos data line 296: [%+v]\n",qosData)
					}
				}
			}
			...
			logger.CtxLog.Infof("5g team This is pcc rule after provisioning of traffic routing info line 334: [%+v]\n",pccRule)
	...
	if ascReqData.EvSubsc != nil {
		logger.PolicyAuthLog.Infof("5g team this is going inside event subscription loop", ascReqData.EvSubsc.NotifUri)
	...
	if ascReqData.AspId != "" && ascReqData.SponId != "" {
		logger.PolicyAuthLog.Infof("5g team this is going inside Initial provisioning of sponsored connectivity information")
	...
	logger.CtxLog.Infof("5g team This is pcf context app session data line 406: [%+v]\n",data)
	...
	if updateSMpolicy {
		...
		// Log the notification being sent to SMF
		if jsonData, err := json.MarshalIndent(notification, "", "  "); err != nil {
			logger.CtxLog.Errorf("Failed to marshal SM Policy Notification: %v", err)
		} else {
			logger.CtxLog.Infof("Sending SM Policy Notification to SMF: %s", string(jsonData))
		}
	
		go p.SendSMPolicyUpdateNotification(smPolicy.PolicyContext.NotificationUri, &notification)
	}
	logger.CtxLog.Infof("5g team This is application session context line 453: [%+v]\n",appSessCtx)
	logger.CtxLog.Infof("5g team This is location header line 454: [%+v]\n",locationHeader)
	return appSessCtx, locationHeader, nil
...
func (p *Processor) HandleDeleteAppSessionContext(
	...
	// 📝 Log entire smPolicy BEFORE deletion
	if appSession.SmPolicyData != nil && appSession.SmPolicyData.PolicyDecision != nil {
		if jsonData, err := json.MarshalIndent(appSession.SmPolicyData.PolicyDecision, "", "  "); err != nil {
			logger.CtxLog.Errorf("❌ Failed to marshal smPolicy before DELETE: %v", err)
		} else {
			logger.CtxLog.Infof("📦 smPolicy.PolicyDecision BEFORE DELETE:\n%s", string(jsonData))
		}
	}
	...
	// 📝 Log entire smPolicy AFTER deletion
	if smPolicy.PolicyDecision != nil {
		if jsonData, err := json.MarshalIndent(smPolicy.PolicyDecision, "", "  "); err != nil {
			logger.CtxLog.Errorf("❌ Failed to marshal smPolicy after DELETE: %v", err)
		} else {
			logger.CtxLog.Infof("🧼 smPolicy.PolicyDecision AFTER DELETE:\n%s", string(jsonData))
		}
	}
...
func (p *Processor) HandleModAppSessionContext(
	...
	// logger.CtxLog.Infof("5g team This is application session context update data line 549: [%+v]\n",appSessionContextUpdateData models.AppSessionContextUpdateData)
	logger.CtxLog.Infof("📦 AppSessionContextUpdateData (raw struct): %+v", appSessionContextUpdateData)
	...
	// Log the appSessionId being searched
	logger.CtxLog.Infof("Searching for AppSessionId: [%s] in AppSessionPool", appSessionId)

	var appSession *pcf_context.AppSessionData
	if val, ok := pcfSelf.AppSessionPool.Load(appSessionId); ok {
		logger.CtxLog.Infof("AppSessionId [%s] found in AppSessionPool: %+v", appSessionId, val)
		appSession = val.(*pcf_context.AppSessionData)

		if jsonData, err := json.MarshalIndent(appSession.AppSessionContext, "", "  "); err == nil {
			logger.CtxLog.Infof("AppSessionContext (JSON): %s", string(jsonData))
		} else {
			logger.CtxLog.Errorf("Failed to marshal AppSessionContext: %v", err)
		}
		// Log the details of AppSessionContext
		logger.CtxLog.Infof("AppSessionContext: %+v", appSession.AppSessionContext)
	
		// Log the details of RelatedPccRuleIds
		logger.CtxLog.Infof("RelatedPccRuleIds: %+v", appSession.RelatedPccRuleIds)
	
		// Log the details of PccRuleIdMapToCompId
		logger.CtxLog.Infof("PccRuleIdMapToCompId: %+v", appSession.PccRuleIdMapToCompId)
	
		// Log the details of Events
		logger.CtxLog.Infof("Events: %+v", appSession.Events)
	
		// Log the details of SmPolicyData
		if appSession.SmPolicyData != nil {
			logger.CtxLog.Infof("SmPolicyData: %+v", appSession.SmPolicyData)
		} else {
			logger.CtxLog.Infof("SmPolicyData is nil")
		}
	} else {
		// Log if the appSessionId is not found
		logger.CtxLog.Warnf("AppSessionId [%s] not found in AppSessionPool", appSessionId)
	}

	if appSession == nil {
		problemDetail := util.GetProblemDetail("can't find app session", util.APPLICATION_SESSION_CONTEXT_NOT_FOUND)
		logger.CtxLog.Errorf("Failed to find AppSessionId [%s]: %+v", appSessionId, problemDetail)
		c.JSON((int)(problemDetail.Status), problemDetail)
		return
	}

	// Log the appSession details after loading
	logger.CtxLog.Infof("Loaded AppSessionId [%s]: %+v", appSessionId, appSession)
	appSessCtx := appSession.AppSessionContext



	logger.CtxLog.Infof("5g team This is application session context update data with bdt ref id line 564: [%+v]\n",appSessionContextUpdateData.BdtRefId)
	...
	smPolicy := appSession.SmPolicyData
	logger.CtxLog.Infof("5g team This is sm policy line 577: [%+v]\n",smPolicy)





		// 🧠 Snapshot current rules for diffing later
	prevPccRules := make(map[string]*models.PccRule)
	for id, rule := range smPolicy.PolicyDecision.PccRules {
		prevPccRules[id] = rule
	}

	prevTcData := make(map[string]*models.TrafficControlData)
	for id, data := range smPolicy.PolicyDecision.TraffContDecs {
		prevTcData[id] = data
	}

	prevQosDecs := make(map[string]*models.QosData)
	for id, data := range smPolicy.PolicyDecision.QosDecs {
		prevQosDecs[id] = data
	}

	// Marshal and print the smPolicy object for debugging
	if jsonData, err := json.MarshalIndent(smPolicy, "", "  "); err != nil {
		logger.CtxLog.Errorf("Failed to marshal smPolicy: %v", err)
	} else {
		logger.CtxLog.Infof("5g team This is the marshalled smPolicy: %s", string(jsonData))
	}
	...
	relatedPccRuleIds := make(map[string]string)
	logger.CtxLog.Infof("5g team This is relatede pcc rule ids line 588: [%+v]\n",relatedPccRuleIds)
	// Event Subscription
	eventSubs := make(map[models.PcfPolicyAuthorizationAfEvent]models.AfNotifMethod)
	updateSMpolicy := false

	if jsonData, err := json.MarshalIndent(appSessionContextUpdateData.MedComponents, "", "  "); err != nil {
		logger.CtxLog.Errorf("Failed to marshal appSessionContextUpdateData.MedComponents: %v", err)
	} else {
		logger.CtxLog.Infof("5g team This is appSessionContextUpdateData.MedComponents line 593: %s", string(jsonData))
	}

	if appSessionContextUpdateData.MedComponents != nil {
		logger.CtxLog.Infof("appSessionContextUpdateData.MedComponents content: %+v", appSessionContextUpdateData.MedComponents)
	} else {
		logger.CtxLog.Infof("appSessionContextUpdateData.MedComponents is nil")
	}

	if appSessionContextUpdateData.MedComponents != nil {
		logger.CtxLog.Infof("appSessionContextUpdateData.MedComponents content: %+v", appSessionContextUpdateData.MedComponents)
	
		// Print all PCC rules in the appSession before processing
		if appSession.SmPolicyData != nil && appSession.SmPolicyData.PolicyDecision != nil {
			logger.CtxLog.Infof("Printing all PCC rules in appSession before processing:")
			printAllPccRules(appSession.SmPolicyData.PolicyDecision.PccRules)
		} else {
			logger.CtxLog.Warnf("No PCC rules found in appSession.")
		}
	
		precedence := getAvailablePrecedence(smPolicy.PolicyDecision.PccRules)
		for compN, medCompRm := range appSessionContextUpdateData.MedComponents {
			medComp := transferMedCompRmToMedComp(medCompRm)
			logger.CtxLog.Infof("Before removeMediaComp: %+v", appSession.RelatedPccRuleIds)
			logger.CtxLog.Infof("MedComponents: %+v", appSession.AppSessionContext.AscReqData.MedComponents)
			// appSession.AppSessionContext.AscReqData.MedComponents
			removeMediaComp(appSession, compN)
			logger.CtxLog.Infof("compN value being sent: %+v",compN)
			...
			logger.CtxLog.Infof("5g team This is medComp.MedType line 610: [%+v]\n",medComp.MedType)

			if medComp.MedType != "" {
				var5qi = util.MediaTypeTo5qiMap[medComp.MedType]
			}
			logger.CtxLog.Infof("5g team This is medComp.MedSubComps line 616: [%+v]\n",medComp.MedSubComps)
			logger.CtxLog.Infof("5g team This is medComp.AfAppId line 633: [%+v]\n",medComp.AfAppId)
			logger.CtxLog.Infof("5g team This is appSessionContextUpdateData.AfAppId. line 639: [%+v]\n",appSessionContextUpdateData.AfAppId)
			if medComp.MedSubComps != nil {
				for _, medSubComp := range medComp.MedSubComps {
			...
			pccRule = util.GetPccRuleByAfAppId(smPolicy.PolicyDecision.PccRules, appID)
			logger.CtxLog.Infof("5g team This is pccRule line 651: [%+v]\n",pccRule)
			...
			if pccRule == nil { // create new pcc rule
				...
				// update qos
				var qosData models.QosData
				logger.CtxLog.Infof("5g team This is qosData line 675: [%+v]\n",qosData)
	...
	logger.CtxLog.Infof("5g team This is appSessionContextUpdateData.AfRoutReq line 702: [%+v]\n",appSessionContextUpdateData.AfRoutReq)
	...
	smPolicy.ArrangeExistEventSubscription()
// Update MedComponents
logger.CtxLog.Infof("MedComponents in update data: %+v", appSessionContextUpdateData.MedComponents)
convertedMedComps := make(map[string]models.MediaComponent)
for compN, medCompRm := range appSessionContextUpdateData.MedComponents {
    convertedMedComps[compN] = *transferMedCompRmToMedComp(medCompRm) // <- fix: dereference here
}
appSessCtx.AscReqData.MedComponents = convertedMedComps

logger.CtxLog.Infof("MedComponents after updating data: %+v", appSessCtx.AscReqData.MedComponents)

// Update AfAppId
logger.CtxLog.Infof("AfAppId in update data: %+v", appSessionContextUpdateData.AfAppId)
appSessCtx.AscReqData.AfAppId = appSessionContextUpdateData.AfAppId
logger.CtxLog.Infof("AfAppId after updating data: %+v", appSessCtx.AscReqData.AfAppId)

// Update AfRoutReq
// logger.CtxLog.Infof("AfRoutReq in update data: %+v", appSessionContextUpdateData.AfRoutReq)
// appSessCtx.AscReqData.AfRoutReq = transferAfRoutReqRmToAfRoutReq(appSessionContextUpdateData.AfRoutReq)
// logger.CtxLog.Infof("AfRoutReq after updating data: %+v", appSessCtx.AscReqData.AfRoutReq)
// 🧹 Track deleted rule IDs
deletedPccRuleIDs := []string{}
for id := range prevPccRules {
	if _, exists := smPolicy.PolicyDecision.PccRules[id]; !exists {
		deletedPccRuleIDs = append(deletedPccRuleIDs, id)
	}
}

deletedTcIDs := []string{}
for id := range prevTcData {
	if _, exists := smPolicy.PolicyDecision.TraffContDecs[id]; !exists {
		deletedTcIDs = append(deletedTcIDs, id)
	}
}

deletedQosIDs := []string{}
for id := range prevQosDecs {
	if _, exists := smPolicy.PolicyDecision.QosDecs[id]; !exists {
		deletedQosIDs = append(deletedQosIDs, id)
	}
}
...
// Send Notification to SMF
	if updateSMpolicy {
		logger.CtxLog.Infof("5g team This is going inside update sm policy loop [%+v]\n",updateSMpolicy)

		smPolicyID := fmt.Sprintf("%s-%d", smPolicy.PcfUe.Supi, smPolicy.PolicyContext.PduSessionId)

		// 🧠 Merge current rules with deletions for notification
		pccRulesToSend := map[string]*models.PccRule{}
		for id, rule := range smPolicy.PolicyDecision.PccRules {
			pccRulesToSend[id] = rule
		}
		for _, id := range deletedPccRuleIDs {
			pccRulesToSend[id] = nil
		}

		tcToSend := map[string]*models.TrafficControlData{}
		for id, data := range smPolicy.PolicyDecision.TraffContDecs {
			tcToSend[id] = data
		}
		for _, id := range deletedTcIDs {
			tcToSend[id] = nil
		}

		qosToSend := map[string]*models.QosData{}
		for id, data := range smPolicy.PolicyDecision.QosDecs {
			qosToSend[id] = data
		}
		for _, id := range deletedQosIDs {
			qosToSend[id] = nil
		}
		...
		notification := models.SmPolicyNotification{
			...
			SmPolicyDecision: &models.SmPolicyDecision{
				PccRules:     pccRulesToSend,
				TraffContDecs: tcToSend,
				QosDecs:      qosToSend,
			},
		}
		...
		if jsonData, err := json.MarshalIndent(notification, "", "  "); err != nil {
			...
			logger.CtxLog.Infof("5g team Sending SM Policy Notification to SMF: %s", string(jsonData))
		}
		logger.CtxLog.Infof("Notification URI: %s", smPolicy.PolicyContext.NotificationUri)

		go p.SendSMPolicyUpdateNotification(smPolicy.PolicyContext.NotificationUri, &notification)
		logger.PolicyAuthLog.Tracef("Send SM Policy[%s] Update Notification", smPolicyID)

		for _, id := range deletedPccRuleIDs {
			delete(smPolicy.PolicyDecision.PccRules, id)
		}
		for _, id := range deletedTcIDs {
			delete(smPolicy.PolicyDecision.TraffContDecs, id)
		}
		for _, id := range deletedQosIDs {
			delete(smPolicy.PolicyDecision.QosDecs, id)
		}
...
func (p *Processor) HandleUpdateEventsSubscContext(
	...
	logger.CtxLog.Infof("5g team This is reponse data of event subcsription of put data line 1003: [%+v]\n",resp)
...
func (p *Processor) handleBDTPolicyInd(pcfSelf *pcf_context.PCFContext,
	...
	logger.CtxLog.Infof("5g team This is application session context response data line 1123: [%+v]\n",respData)
...
func getFlowInfos(subComp *models.MediaSubComponent) ([]models.FlowInformation, error) {
	...
	// logger.CtxLog.Infof("5g team This is flow info line 1323: [%+v]\n",flowInfo)
	return flows, nil
}
...
func updateQosInMedComp(qosData models.QosData, comp *models.MediaComponent) (models.QosData,
	...
	logger.CtxLog.Infof("5g team This is updated qos data line 1471: [%+v]\n",updatedQosData)
...
func updateQosInMedSubComp(qosData *models.QosData, comp *models.MediaComponent,
	...
	for _, flow := range subsComp.FDescs {
		...
		if subsComp.FlowUsage != models.FlowUsage_RTCP {
			// not RTCP
			logger.CtxLog.Infof("5g team This is not RTCP")

			if both || dir == models.FlowDirection_UPLINK {
				logger.CtxLog.Infof("5g team This is uplink")
				ulExist = true
				if comp.MarBwUl != "" {
					bwUl, err := pcf_context.ConvertBitRateToKbps(comp.MarBwUl)
					if err != nil {
					...
					maxBwUl += bwUl
					logger.CtxLog.Infof("5g team This is maxBwUl[%+v], bwUl[%+v]\n", maxBwUl, bwUl)
				...
				if comp.MirBwUl != "" {
					...
					minBwUl += bwUl
					logger.CtxLog.Infof("5g team This is minBwUl[%+v], bwUl[%+v]\n", minBwUl, bwUl)

				}
			}
			if both || dir == models.FlowDirection_DOWNLINK {
				logger.CtxLog.Infof("5g team This is downlink")
				dlExist = true
				if comp.MarBwDl != "" {
					bwDl, err := pcf_context.ConvertBitRateToKbps(comp.MarBwDl)
					if err != nil {
					...
					maxBwDl += bwDl
					logger.CtxLog.Infof("5g team This is maxBwDl[%+v], bwDl[%+v]\n", maxBwDl, bwDl)
				}
				if comp.MirBwDl != "" {
					bwDl, err := pcf_context.ConvertBitRateToKbps(comp.MirBwDl)
					if err != nil {
					...
					minBwDl += bwDl
					logger.CtxLog.Infof("5g team This is minBwDl[%+v], bwDl[%+v]\n", minBwDl, bwDl)
				}
			}
		} else {
			// RTCP
			logger.CtxLog.Infof("5g team This is RTCP")
			if both || dir == models.FlowDirection_UPLINK {
				logger.CtxLog.Infof("5g team This is uplink")
				ulExist = true
				if subsComp.MarBwUl != "" {
	...
	// start1: makes changes here to update the QoS Data

	updatedQosData.MaxbrDl = comp.MarBwDl
	updatedQosData.MaxbrUl = comp.MarBwUl
	updatedQosData.GbrDl = comp.MirBwDl
	updatedQosData.GbrUl = comp.MirBwUl
	...
	// if maxBwDl == 0.0 {
	// 	updatedQosData.MaxbrDl = comp.MarBwDl
	// 	logger.CtxLog.Infof("5g team This is updated comp maxBwDl[%+v]\n", updatedQosData.MaxbrDl)
	// } else {
	// 	updatedQosData.MaxbrDl = pcf_context.ConvertBitRateToString(maxBwDl)
	// 	logger.CtxLog.Infof("5g team This is updated pcf_context converted maxBwDl[%+v]\n ", updatedQosData.MaxbrDl)

	// }
	// // update Uplink MBR
	// if maxBwUl == 0.0 {
	// 	updatedQosData.MaxbrUl = comp.MarBwUl
	// 	logger.CtxLog.Infof("5g team This is updated comp maxBwUl[%+v]\n", updatedQosData.MaxbrUl)
	// } else {
	// 	updatedQosData.MaxbrUl = pcf_context.ConvertBitRateToString(maxBwUl)
	// 	logger.CtxLog.Infof("5g team This is updated pcf_context converted maxBwUl[%+v]\n ", updatedQosData.MaxbrUl)
	// }
	// // if gbr == 0 then assign gbr = mbr
	// // update Downlink GBR
	// if minBwDl != 0.0 {
	// 	updatedQosData.GbrDl = pcf_context.ConvertBitRateToString(minBwDl)
	// 	logger.CtxLog.Infof("5g team This is updated pcf_context i.e minBwDl converted gbrDl[%+v]\n ", updatedQosData.GbrDl)
	// }
	// // update Uplink GBR
	// if minBwUl != 0.0 {
	// 	updatedQosData.GbrUl = pcf_context.ConvertBitRateToString(minBwUl)
	// 	logger.CtxLog.Infof("5g team This is updated pcf_context i.e minBwUl converted gbrUl[%+v]\n ", updatedQosData.GbrUl)
	// }
	
	// 	logger.CtxLog.Infof("5g team This is updated Updated QoS Data: %+v, UL Exist: %+v, DL Exist: %+v\n", updatedQosData, ulExist, dlExist)
...
func removeMediaComp(appSession *pcf_context.AppSessionData, compN string) {
	logger.CtxLog.Infof("Starting removeMediaComp for compN: %s", compN)

	idMaps := appSession.RelatedPccRuleIds
	logger.CtxLog.Infof("RelatedPccRuleIds: %+v", idMaps)

	smPolicy := appSession.SmPolicyData
	logger.CtxLog.Infof("SmPolicyData: %+v", smPolicy)

	if idMaps != nil {
		logger.CtxLog.Infof("idMaps is not nil")
		// logAsJSON("appSession.AppSessionContext.AscReqData.MedComponents",appSession.AppSessionContext.AscReqData.MedComponents)
		if appSession.AppSessionContext.AscReqData.MedComponents == nil {
			logger.CtxLog.Warnf("MedComponents is nil in AscReqData")
			return
		}
		...
		comp, exist := appSession.AppSessionContext.AscReqData.MedComponents[compN]
		logger.CtxLog.Infof("Checking existence of compN: %s, exist: %v, comp: %+v", compN, exist, comp)

		if !exist {
			logger.CtxLog.Warnf("Component %s does not exist", compN)
			return
		}
	if comp.MedSubComps != nil {
    logger.CtxLog.Infof("Processing Media SubComponents for compN: %s", compN)
    for fNum := range comp.MedSubComps {
        key := fmt.Sprintf("%s-%s", compN, fNum)
        logger.CtxLog.Infof("Generated key for Media SubComponent: %s", key)

        pccRuleID := idMaps[key]
        logger.CtxLog.Infof("Retrieved PCC Rule ID for key %s: %s", key, pccRuleID)

        err := smPolicy.RemovePccRule(pccRuleID, nil)
        if err != nil {
            logger.PolicyAuthLog.Warnf("Failed to remove PCC Rule ID %s: %v", pccRuleID, err)
        } else {
            logger.CtxLog.Infof("Successfully removed PCC Rule ID: %s", pccRuleID)
        }

        delete(appSession.RelatedPccRuleIds, key)
        logger.CtxLog.Infof("Deleted RelatedPccRuleIds entry for key: %s", key)

        delete(appSession.PccRuleIdMapToCompId, pccRuleID)
        logger.CtxLog.Infof("Deleted PccRuleIdMapToCompId entry for PCC Rule ID: %s", pccRuleID)
    }
} else {
    logger.CtxLog.Infof("No Media SubComponents found for compN: %s", compN)

    pccRuleID := idMaps[compN]
    logger.CtxLog.Infof("Retrieved PCC Rule ID for compN %s: %s", compN, pccRuleID)
    ...
    delete(appSession.RelatedPccRuleIds, compN)
    logger.CtxLog.Infof("Deleted RelatedPccRuleIds entry for compN: %s", compN)
...
delete(appSession.AppSessionContext.AscReqData.MedComponents, compN)
logger.CtxLog.Infof("Deleted MedComponents entry for compN: %s", compN)}
}

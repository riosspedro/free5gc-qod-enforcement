...
const (
	...
	ServiceQos string ="qos"
	ServiceOuth string = "oauth2" // OAuth service for NEF
)
...
const (
	...
	AsSessionWithQoSResUriPrefix = "/" + ServiceAsSessionWithQoS + "/v1"
	QosResUriPrefix = "/" + ServiceQos + "/v1"
	NefOuthResUriPrefix = "/" + ServiceOuth
)
...
type Tls struct {
	...
	Ca  string `yaml:"ca,omitempty" valid:"type(string),optional"`
}
...
func (c *Config) TLSCaPath() string {
	c.RLock()
	defer c.RUnlock()
	if c.Configuration.Sbi.Tls != nil && c.Configuration.Sbi.Tls.Ca != "" {
		return c.Configuration.Sbi.Tls.Ca
	}
	return "cert/ca-cert.pem" // default fallback path if not explicitly set
...
func (c *Config) ServiceUri(name string) string {
	...
	case ServiceAsSessionWithQoS:
		return c.SbiUri() + AsSessionWithQoSResUriPrefix
	case ServiceQos:
		return c.SbiUri() + QosResUriPrefix
	case ServiceOuth:
		return c.SbiUri() + NefOuthResUriPrefix
	default:
		return ""
	}
}
